...
<body>
	...
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="">
			<h2 style="color:orange;margin-left:20%;">Delete building</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Enter building:</span>
				<input type="text" name="building"><br/><br/>
				</div>
				<input type="submit" value="DELETE" class="search-btn">
				<a href="building-report.jsp"><input type="button" value="BACK" class="back-btn"></a>
		...
		if(request.getParameter("building")!=null)
			{
				...
				stmt=con1.prepareStatement("delete from building_db where bno=?");
				stmt.setString(1,db_id);
				i=stmt.executeUpdate();
				if(i>0)
					{%>
						<script type='text/javascript'>
						alert('Building is deleted');
						window.location.href='building-report.jsp';
						session.invalidate();
						</script>
					<%}
					else
					{%>
					<script type='text/javascript'>
					alert('Building is not deleted enter valid building name');
					window.location.href='building-report.jsp';
