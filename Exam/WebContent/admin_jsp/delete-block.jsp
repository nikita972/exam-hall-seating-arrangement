...
<div class="container">
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="">
			<h2 style="color:orange;margin-left:20%;">Delete block</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Enter block:</span>
		...
		if(request.getParameter("building")!=null)
			{
				...
				stmt=con1.prepareStatement("delete from block_db where bno=?");
				stmt.setString(1,db_id);
				i=stmt.executeUpdate();
				if(i>0)
				{%>
					<script type='text/javascript'>
					alert('Block is deleted');
					window.location.href='block-report.jsp';
					session.invalidate();
					</script>
				<%}
				else
				{%>
				<script type='text/javascript'>
				alert('Block is not deleted enter valid block name');
				window.location.href='block-report.jsp';
				session.invalidate();
				</script>
					<%}
