...
<div class="container">
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="">
			<h2 style="color:orange;margin-left:20%;">Delete Floor</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Enter Floor:</span>
				<input type="text" name="building"><br/><br/>
				</div>
				<input type="submit" value="DELETE" class="search-btn">
				<a href="floor-report.jsp"><input type="button" value="BACK" class="back-btn"></a>
		...
		if(request.getParameter("building")!=null)
			{
				...
				stmt=con1.prepareStatement("delete from floor_db where id=?");
				stmt.setString(1,db_id);
				i=stmt.executeUpdate();
				if(i>0)
				{%>
					<script type='text/javascript'>
					alert('Floor is deleted');
					window.location.href='floor-report.jsp';
					session.invalidate();
					</script>
				<%}
				else
				{%>
				<script type='text/javascript'>
				alert('floor is not deleted enter valid floor name');
				window.location.href='floor-report.jsp';
				session.invalidate();
				</script>
					<%}
