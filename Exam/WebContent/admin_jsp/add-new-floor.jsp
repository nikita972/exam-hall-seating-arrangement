...
<div class="navbar_css">
	...
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="add-new-floor.jsp" method="post">
				<h2 style="color:orange;margin-left:20%;">Add new floor</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Floor title:</span>
				<input type="text" name="floor" placeholder="floor1"><br/><br/>
				</div>
		...
		%>
			...
			if(request.getParameter("floor")!=null)
			{
				...
				stmt=con1.prepareStatement("select * from floor_db");
				rs=stmt.executeQuery();
				while(rs.next())
				{
					db_floor=rs.getString(2);
					if(floor.equals(db_floor))
					{
						f=1;
					}
				}
				...
				{%>
					...
					alert('floor is already added.please add new floor');
					window.location.href='add-new-floor.jsp';
					</script>
				<%}
				else
				{
				floor=request.getParameter("floor");
				stmt=con1.prepareStatement("insert into floor_db(floor)values(?)");
				stmt.setString(1,floor);
				int i=stmt.executeUpdate();
				if(i>0)
				{%>
					<script type='text/javascript'>
					alert('Floor added sucessfully');
					window.location.href='add-new-floor.jsp';
					</script>
				<%}
				else
				{%>
					<script type='text/javascript'>
					alert('Floor not added sucessfully');
					window.location.href='add-new-floor.jsp';
					</script>
			<%}
			}
