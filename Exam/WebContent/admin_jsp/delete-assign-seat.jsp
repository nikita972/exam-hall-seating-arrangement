...
<div class="container">
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="">
			<h2 style="color:orange;margin-left:20%;">Delete Assign seat</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Enter Reg no:</span>
				<input type="text" name="building"><br/><br/>
				</div>
				<input type="submit" value="DELETE" class="search-btn">
				<a href="assign-seat-report.jsp"><input type="button" value="BACK" class="back-btn"></a>
		</form>
		</div>
		</div>
		</div>
		...
		if(request.getParameter("building")!=null)
			{
				...
				while(rs.next())
				{
					...
					{
						db_id=rs.getString(2);
					}
				}
				stmt=con1.prepareStatement("delete from assign_seat_db where reg_no=?");
				stmt.setString(1,db_id);
				i=stmt.executeUpdate();
				if(i>0)
				{%>
					<script type='text/javascript'>
					alert('Assign seat is deleted');
					window.location.href='assign-seat-report.jsp';
					session.invalidate();
					</script>
				<%}
				else
				{%>
				<script type='text/javascript'>
				alert('Assign seat is not deleted enter valid registration number of student');
				window.location.href='assign-seat-report.jsp';
				session.invalidate();
				</script>
					<%}
