...
<div class="container">
	<div class="row" >
		...
		<div class="col-md-7" style="background-color: aliceblue;margin-top:40px;">
			<form action="">
			<h2 style="color:orange;margin-left:20%;">Delete Seat</h2><br/>
				<div class="textbox-search">
				<span class="span-search">Enter seat:</span>
				<input type="text" name="building"><br/><br/>
				</div>
				<input type="submit" value="DELETE" class="search-btn">
				<a href="seat-report.jsp"><input type="button" value="BACK" class="back-btn"></a>
				</form>
		</div>
		</div>
		</div>
		...
		if(request.getParameter("building")!=null)
			{
				...
				stmt=con1.prepareStatement("delete from seat_db where sno=?");
				stmt.setString(1,db_id);
				i=stmt.executeUpdate();
				if(i>0)
				{%>
					<script type='text/javascript'>
					alert('Seat is deleted');
					window.location.href='seat-report.jsp';
					session.invalidate();
					</script>
				<%}
				else
				{%>
				<script type='text/javascript'>
				alert('Seat is not deleted enter valid seat name');
				window.location.href='seat-report.jsp';
				session.invalidate();
				</script>
					<%}
